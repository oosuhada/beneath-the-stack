...
| File durability boundary | OS boundary lab separates `write()` completion from `fsync()` completion | `browser-reliability-runtime/src/local-llm/queue.ts` | **Applied:** queue publish now writes a temp file, calls `datasync()`, atomically renames it into place, and best-effort syncs the queue directory. |
...
| Optional durable append | OS/fsync lab asks when an append crosses a durability boundary | `memory-atlas-server/src/MessageHistory.cpp`, `tests/test_message_history.cpp` | **Applied:** history logging uses a POSIX append loop and can opt into `fsync` through `CHERRY_HISTORY_DURABLE_WRITES=1`; default writes do not silently claim crash durability. |
...
| Firmware timing / bounded buffers / protocol framing | `firmware-boundary` lab: ring buffer, serial frame parser, cooperative scheduler, simulated MMIO, fault recovery | `iBridge-Studio/docs/DEVICE_BOUNDARY_NOTES.md` | **Product bridge:** iBridge is not firmware, but its virtual-display capture/encode/send/receiver path has the same queue-depth, frame-pacing, protocol and safe-recovery questions. Runtime changes are deferred until queue-depth/presentation timestamps are measured. |
